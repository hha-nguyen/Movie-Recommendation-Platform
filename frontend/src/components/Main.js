...
class MainPage extends Component {
  ...
  getAllMoviesByGenre = async () => {
    for (let i = 0; i < this.state.genre.length; i++) {
      //debugger;
      await MovieApiService.getMoviesByGenre(i+1)
        .then((res) => {
          switch (i) {
            case 0: // action
            ...
            case 1: // adventure
              ...
              break;
            case 2: // comedy
              this.setState({ comedys: res.data }, () => {});
              break;
            case 3: // documentary
              this.setState(
                { documentarys: res.data },
                () => {}
              );
              break;
            case 4: // family
              this.setState({ familys: res.data }, () => {});
              break;
            case 5: // fantasy
              this.setState({ fantasys: res.data }, () => {});
              break;
            case 6: // horror
              this.setState({ horrors: res.data }, () => {});
              break;
            default:
              break;
          }
        ...
        .catch((err) => {
          console.error("getMoviesByGenre get error : ", err);
        });
    }
  };
  getMainPopularMovies = async () => {
    await MovieApiService.getPopularMovies(1)
      .then((res) => {
